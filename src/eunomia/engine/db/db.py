...
def init_db(sql_database_url: str):
    ...
    if not sql_database_url:
        raise ValueError("ENGINE_SQL_DATABASE_URL must be provided for policy engine")
    ...
    if sql_database_url.startswith("sqlite"):
        ...
        if db_url.database and db_url.database != ":memory:":
            ...
            if not db_dir.exists():
                # If the database is the default path, create the directory
                if (
                    sql_database_url
                    == Settings.model_fields["ENGINE_SQL_DATABASE_URL"].default
                ):
