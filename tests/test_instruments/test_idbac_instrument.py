...
def test_idbac_instrument_run_different_ids(
    idbac_instrument: IdbacInstrument, pii_sample_text: str
) -> None:
    # When user_id and doc_id are different, the nested instruments should run
    result = idbac_instrument.run(pii_sample_text, user_id="user1", doc_id="user2")
...
def test_idbac_instrument_run_same_ids(
    ...
    result = idbac_instrument.run(pii_sample_text, user_id="user1", doc_id="user1")
...
def test_idbac_instrument_run_missing_ids(
    ...
    result = idbac_instrument.run(pii_sample_text)
    assert "john.doe@example.com" not in result
