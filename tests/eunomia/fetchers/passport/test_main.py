...
class TestPassportFetcher:
    ...
    def test_post_init_with_registry_success(self, mock_get_fetcher, passport_config_with_registry, mock_registry_fetcher):
    ...
    @patch.object(FetcherFactory, 'get_fetcher')
    def test_post_init_with_registry_failure(self, mock_get_fetcher, passport_config_with_registry):
        ...
        with pytest.raises(ValueError, match="Passport requires registry but registry fetcher is not initialized"):
            fetcher.post_init()
    ...
    def test_issue_passport_basic(self, passport_fetcher, sample_uri, sample_attributes):
    ...
    def test_issue_passport_with_registry(self, passport_fetcher_with_registry, sample_uri, sample_attributes):
        ...
        passport_fetcher_with_registry._registry.get_entity.assert_called_once_with(sample_uri)
        ...
        expected_attributes = {**sample_attributes, **{"name": "Test Resource", "type": "document", "owner": "user123"}}
        assert decoded['attr'] == expected_attributes

    def test_issue_passport_registry_entity_not_found(self, passport_fetcher_with_registry, sample_uri):
        ...
        with pytest.raises(ValueError, match="Passport cannot be issued for non-registered entity"):
            passport_fetcher_with_registry.issue_passport(sample_uri)

    @patch('uuid.uuid4')
    def test_issue_passport_deterministic_id(self, mock_uuid, passport_fetcher, sample_uri):
    ...
    def test_issue_passport_logging_success(self, mock_logger, passport_fetcher, sample_uri):
    ...
    @patch('eunomia.fetchers.passport.main.logger')
    def test_issue_passport_logging_failure(self, mock_logger, passport_fetcher_with_registry, sample_uri):
    ...
    def test_verify_passport_valid(self, passport_fetcher, valid_jwt_token, valid_passport_jwt):
    ...
    @pytest.mark.asyncio
    async def test_fetch_attributes_valid_token(self, passport_fetcher, valid_jwt_token, valid_passport_jwt):
        """Test fetching attributes with valid token"""
        attributes = await passport_fetcher.fetch_attributes(valid_jwt_token)
        
        expected_attributes = {
            **valid_passport_jwt.attr,
            "sub": valid_passport_jwt.sub
        }
        assert attributes == expected_attributes

    @pytest.mark.asyncio
    async def test_fetch_attributes_invalid_token(self, passport_fetcher, invalid_jwt_token):
        """Test fetching attributes with invalid token raises error"""
        with pytest.raises(JWTError):
            await passport_fetcher.fetch_attributes(invalid_jwt_token)

    @pytest.mark.asyncio
    async def test_fetch_attributes_expired_token(self, passport_fetcher, expired_jwt_token):
        """Test fetching attributes with expired token raises error"""
        with pytest.raises(JWTError):
            await passport_fetcher.fetch_attributes(expired_jwt_token)

    @pytest.mark.asyncio
    async def test_fetch_attributes_empty_attributes(self, passport_fetcher, passport_config):
    ...
    def test_integration_issue_and_verify(self, passport_fetcher, sample_uri, sample_attributes):
    ...
    @pytest.mark.asyncio
    async def test_integration_issue_and_fetch_attributes(self, passport_fetcher, sample_uri, sample_attributes):
    ...
    def test_passport_expiration_timing(self, passport_fetcher, sample_uri):
        ...
        assert time_diff <= 2, f"Expected expiration around {expected_exp}, got {actual_exp} (diff: {time_diff}s)"
