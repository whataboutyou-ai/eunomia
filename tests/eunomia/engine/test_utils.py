...
def test_create_attribute_condition():
    ...
    assert condition.operator == schemas.ConditionOperator.EQUALS
    assert condition.value == "admin"

    condition = utils.create_attribute_condition(
        "role", "admin", operator=schemas.ConditionOperator.CONTAINS
    )
    assert condition.operator == schemas.ConditionOperator.CONTAINS
...
def test_create_simple_policy():
    ...
    assert policy.default_effect == schemas.PolicyEffect.DENY
    ...
    assert principal_condition.operator == schemas.ConditionOperator.EQUALS
    ...
    assert resource_condition.operator == schemas.ConditionOperator.EQUALS
